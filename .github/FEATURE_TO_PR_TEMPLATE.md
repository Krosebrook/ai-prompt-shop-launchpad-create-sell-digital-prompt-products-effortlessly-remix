...
#### 1.1 — Files to Change
```
- src/components/NewFeature.tsx (NEW)
...
- **State**: React hooks and context (built-in)
...
- **Styling**: Tailwind utility classes with custom theme (dark mode enabled)
...
- ⚠️ **Blink SDK**: Uses @blinkdotnew/sdk - check compatibility when adding features

**Build Commands**:
```bash
npm run dev       # Development server (Vite)
npm run build     # Production build
npm run lint      # Run all linters (types, js, css, css-vars, css-classes)
...
- `.github/FEATURE_TO_PR_TEMPLATE.md` (this document)
- `package.json` (dependencies and scripts)
- `tailwind.config.cjs` (theme configuration)
- `src/components/ui/*` (UI component library structure)

**Helpful Context**:
- Landing page sections: Hero, Why Now, Window, How It Works, What's Inside, Pricing, Creator Note, Launch Week, Social Proof, FAQ, Final CTA, Footer
- 12 main section components
- 60+ UI components in `src/components/ui/` (Radix UI wrappers)
- Focus on selling AI Prompt Shop Launchpad product
- Target audience: Coaches, creators, and service providers

**Component Structure**:
- **Main sections**: `src/components/*Section.tsx` or standalone components
- **UI primitives**: `src/components/ui/*` (reusable components)
- **Utilities**: `src/lib/utils.ts` (helper functions, cn() for class merging)
- **Hooks**: `src/hooks/*` (custom React hooks if any)
...
### ✅ Good Definition
```
Feature: Prompt category filter
Outcome: Users can filter prompts shown in the "What's Inside" section 
by category (Marketing, SEO, Social, etc.) with instant visual feedback.

Scope Level: 🟢 Medium

Acceptance Criteria:
- [ ] Filter buttons above prompts section
- [ ] Clicking filter shows/hides relevant prompts
...
- Tests are passing locally (if they exist)
...
If stuck or unsure: 
1. **Check documentation**:  README, package.json, existing components
...
**Repository**:  `ai-prompt-shop-launchpad-create-sell-digital-prompt-products-effortlessly-remix`
